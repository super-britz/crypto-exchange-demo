...
K 线使用 `lightweight-charts` 绘制，数据结构包括 open、high、low、close、volume。启动时拉取最近 72 根 1m K 线，之后由 WebSocket 增量更新最新 candle。

对应代码：`src/app.js` 里的 `initChart()`、`renderKline()`、`loadInitialCandles()`、`upsertStreamCandle()`。
