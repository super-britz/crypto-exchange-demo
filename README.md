...
5. 在下单区选择市价单，点击开仓，观察账户资产面板里的可用余额、占用保证金、钱包余额、总权益变化。
...
10. 点击“手动平仓”后查看成交记录，确认平仓价、手续费和已实现盈亏被记录下来。
11. 点击“暴跌模拟”，临时切到本地压力测试行情，让强平更容易被演示出来。
12. 如果价格触发强平线，系统会自动强平并写入事件日志，同时移除 K 线上的强平价线。
...
开仓会占用保证金并扣交易手续费；撤单会释放委托保证金；平仓会释放持仓保证金并落地已实现盈亏。资金费率支付会减少可用余额，收取则增加可用余额。

对应代码：`getAccountSnapshot()`、`renderAccount()`、`calcUsedMargin()`。

成交记录会保留最近 12 笔开仓、平仓和强平流水。开仓记录展示成交价、数量和开仓手续费；平仓和强平记录会展示本轮净已实现盈亏：

```text
净已实现盈亏 = 毛盈亏 - 开仓手续费 - 平仓手续费 - 资金费累计
```

对应代码：`addTrade()`、`renderTradeHistory()`、`createPositionFromOrder()`、`closePosition()`、`checkLiquidation()`。
