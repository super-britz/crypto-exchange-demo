...
4. 在下单区选择市价单，点击开仓，观察仓位区出现入场价、标记价、强平价、未实现盈亏。
5. 切到限价单，输入委托价，点击挂单，观察“当前委托”区域。
6. 等行情到价自动成交，或者点击撤单释放保证金。
7. 成交后看 K 线图上出现 `Liq` 强平价线。
8. 点击“暴跌模拟”，临时切到本地压力测试行情，让强平更容易被演示出来。
9. 如果价格触发强平线，系统会自动强平并写入事件日志，同时移除 K 线上的强平价线。
10. 也可以点击“手动平仓”观察余额和日志变化。
